    ...
    const fetchUserData = async () => {
      ...
      const HandleDeleteGrade = async (userId) => {
        try {
          ...
          await axios.delete(
            `http://localhost:5000/grade/delete-grade/${userId}`,
            config
          );
      ...
      <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200 pl-60">
        ...
        <div className="ml-4 mt-2">
          ...
          <button className=" bg-blue-500  font-bold py-2 px-4 rounded-md">
            <Link to="/AddGrade" className="text-white no-underline">Add Grade</Link>
          </button>
        </div>
                ...
                data.map((item) => {
                 ...
                 <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                     ...
                     <td class=" pl-16 space-x-2 py-4">
                         <button  className="font-medium text-blue-600 dark:text-blue-500 no-underline  hover:no-underline" onClick={() =>HandleEditGrade(item._id)}>Edit</button>
                         <button  className="font-medium text-red-600 no-underline hover:text-red-600 hover:no-underline" onClick={() =>HandleDeleteGrade(item._id)}>Delete</button>
                     </td>
                 </tr>
                     )
                    })
